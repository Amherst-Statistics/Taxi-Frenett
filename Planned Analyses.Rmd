...
```{r}
censusdata <- read.csv("http://www.frenett.net/nyc/data/censusdata.csv")
xyplot(tips ~ log(hhincome), data = censusdata, type = c("p", "r"))
lm <- lm(tips ~ log(hhincome), data = censusdata)
plot(lm)
summary(lm)
```
...
dat <- data.frame(doy = 1:365, avg_tip = as.numeric(dat))
